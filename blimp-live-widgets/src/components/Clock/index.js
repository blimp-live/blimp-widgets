  ...
  render() {
    ...
    return (
      <div className = {styles.wrapper}>
        ...
        <p className = {styles.clock}>{this.state.hour == 12 || this.state.hour == 0 ? 12 : this.state.hour%12}:{('0' + this.state.minutes).slice(-2)}:{('0' + this.state.seconds).slice(-2)} { this.state.hour/12 > 0 ? "PM": "AM" }</p>
      </div>
    )
  }
}
